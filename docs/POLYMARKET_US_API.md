...
**Status:** ADAPTER + BURN-IN HARDENING BUILT — USRetailAdapter, identifier registry, state model, raw collector, US phase machine, Direct Exchange refdata layer, gRPC stream consumer, reconciliation layer, AND the US burn-in hardening (`UsHealthTracker`, US-specific gates, US provenance incl. instrument-snapshot hash, US burn-in report with US-specific failure counters, frozen baseline config `config/frozen/us-v0.4.0-baseline.yaml`). Qualifying path ready: US_BASELINE_FROZEN → US_BURNIN_RUNNING → live collection → fault injection → reconciliation → A/B replay → US FINAL GATE: PASS → US_BURNIN_PASSED → REAL_DATA_START_US → US_COLLECTION_RUNNING.
