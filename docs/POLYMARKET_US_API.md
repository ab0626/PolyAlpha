...
**Status:** ADAPTER BUILT — USRetailAdapter, identifier registry, state model, raw collector, US phase machine, Direct Exchange refdata layer, gRPC market-data stream consumer, AND the reconciliation layer (`UsReconciliationReport`: four-question identity/semantics/structure/freshness comparison, reason codes, canonical-book-only comparison, authority hierarchy gRPC↔Exchange REST primary + retail secondary) are implemented and offline-verified. US burn-in hardening is the documented next phase.
